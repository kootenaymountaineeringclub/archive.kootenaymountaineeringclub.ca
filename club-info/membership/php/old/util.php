...
//	require_once 'constants.php';
	...
	function MembershipYearSelect ()
	{
		...
		$lapseDate = 210;  // February 9th of any year. If not past that date, they get benefits
	...
	function debug_log($msg)
	{
	 	if (DEBUG_SYSLOG) 
	 	{
	    	...
	    	$time = date('H:i:s');  
	    	$today = date('Y-m-d');
			$fp = fopen("log/php.log", "a") or die("Cannot open log file");
			fwrite($fp, "$today $time  $msg\n");  
			fclose($fp);
		}
	}
...
if (isset($_REQUEST['paymentMethod']))
	...
	function db_findMembers()
	{
		...
		$sql = "SELECT * FROM Member WHERE Year = " . $_REQUEST['year'];
...
//		debug_log("escapeXml: " . $text);
	...
	function html_kmc_logo()
	{
		...
		$display_block .= "</td>";
		$display_block .= "<td>";
		$display_block .= "<p><a href='http://www.kootenaymountaineering.bc.ca/'>Return to Kootenay Mountaineering Club main page</a></p>";
